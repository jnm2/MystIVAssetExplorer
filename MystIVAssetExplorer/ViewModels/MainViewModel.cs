...
public class MainViewModel : ViewModelBase, IDisposable
{
    ...
    private void UpdateSkyboxModel()
    {
        ...
        var cubeFolder = PickSkyboxFolder(SelectedAssetBrowserNode);

        NodeViewerSkyboxModel = cubeFolder is null
            ? null
            : new ReferenceCountedDisposable<SkyboxModel>(
                SkyboxModel.FromFiles(cubeFolder.FolderListing),
                out nodeViewerBoxModelLease);

        originalBoxModelLease?.Dispose();

        static AssetBrowserNode? PickSkyboxFolder(AssetBrowserNode? searchRoot)
        {
